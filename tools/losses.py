...
class DecadeCELoss(nn.Module):
    def __init__(self, sigma = 5) -> None:
        super().__init__()
        self.ce = nn.MultiLabelSoftMarginLoss()
        self.sigma = sigma
    ...
    def forward(self, input, target):
        ...
        # target: B
        B = target.size(0)
        target_decade = 1 / (self.sigma * (2 * torch.pi)**.5) * \
                            torch.exp(-0.5 * ((torch.linspace(5, 95, 10).repeat(B).cuda().view(B,-1) - target.unsqueeze(1)) / self.sigma) ** 2)
        target_decade /= target_decade.max()
        loss = self.ce(input, target_decade)
...
class HuberLoss(nn.Module):
    ...
    def forward(self, y_pred, y_true):
        diff = torch.abs(y_true.view(-1) - y_pred.view(-1))
        loss = torch.where(diff < 5, 5 * (diff - 2.5), 0.5 * (diff ** 2))
